...
public class RedBlackBSTInteger<Value> implements Serializable {
    ...
    public Iterable<Integer> values() {
        ...
        return values(min(root).key, max());
    }
    ...
    // to the queue
    private void values(Node x, Queue<Integer> queue, int lo, int hi) {
        if (x == null) return;
        if (lo < x.key) values(x.left, queue, lo, hi);
        if (lo <= x.key && hi >= x.key){
            queue.enqueue(x.key);
        }
        if (hi > x.key) values(x.right, queue, lo, hi);
    }
}
